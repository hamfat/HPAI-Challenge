...
# Reactive culling (completed, delayed by 3 days)
reactive_culls <- cases %>%
  filter(cull_status == "completed",
         !is.na(cull_start)) %>%
  mutate(
    cull_start_shifted = cull_start + 3
  ) %>%
  select(farm_id, cull_start = cull_start_shifted)

# Preventive culling (completed only, no delay)
preventive_culls <- prev_culls %>%
  filter(cull_status == "completed",
         !is.na(cull_start)) %>%
  select(farm_id, cull_start)

# Combine
all_culls <- bind_rows(
  reactive_culls   %>% mutate(type = "reactive"),
  preventive_culls %>% mutate(type = "preventive")
)
...
run_seir_county <- function(beta_within, beta_between, sigma, gamma,
  ...
  for (t in 2:n_days) {
    ...
    remove_S <- pmin(S_c[,t], to_cull)
    S_c[,t] <- S_c[,t] - remove_S
    to_cull <- to_cull - remove_S
    ...
    R_c[,t] <- R_c[,t] + remove_S + remove_E + remove_I
  }
...
beta_within_grid  <- c(0.1, 0.2, 0.3)
beta_between_grid <- c(0.05, 0.1, 0.2)
...
p_I_prod <- ggplot(
  ...
  labs(
    title = "Production-Specific Infectious Farms",
    x = "Date",
    y = "Number of infectious farms",
    color = "Production",
    fill  = "Production"
...
ggplot(I_county_ci, aes(date, mean)) +
  ...
  theme_minimal() +
  labs(
    title = "County-level infectious farms (ensemble mean + 95% CI)",
    x = "Date",
    y = "Number of infectious farms"
  )
...
p_facet2 <- ggplot(
  ...
  scale_fill_viridis_c(
    option = "C",
    breaks = seq(0, max(I_county_map$mean, na.rm = TRUE), by = 5),
    labels = seq(0, max(I_county_map$mean, na.rm = TRUE), by = 5)
  ) +
  facet_wrap(~ date) +
  theme_pub() +
  theme(
...
p_compare_inc <- ggplot(compare_inc, aes(date)) +
  ...
  geom_vline(xintercept = fit_end_date,
             linetype = "dashed", color = "grey30") +
  labs(
    title = "Ensemble-Predicted Incidence vs Observed Incidence",
    subtitle = "Daily new infectious farms: ensemble mean + 95% CI vs confirmed cases",
    x = "Date",
    y = expression("New infectious farms per day (" * Delta * I * ")")
  ) +
  theme_pub() +
  theme(
...
ggsave(
  "ensemblevsobserved_incidence.pdf",
  plot = p_compare_inc,
  width = 10,
  height = 6,
  device = cairo_pdf
...
p_county_compare <- ggplot(county_ts_compare_ci, aes(date)) +
  ...
  labs(
    title = "County-level Infectious Farms: Actual vs Ensemble (95% CI)",
    x = "Date",
    y = "Number of infectious farms",
    caption = "Black = Actual | Red = Ensemble mean | Ribbon = 95% CI"
  )

print(p_county_compare)

ggsave(
  "Icountyactualvensemble.pdf",
...
p_species_risk <- ggplot(
  ...
  scale_fill_manual(values = species_cols) +
  labs(
    title = "Approximate Species-Specific Risk in Top-Risk Counties",
    x = "Species",
...
p_production_risk <- ggplot(
  ...
  scale_fill_manual(values = production_cols) +
  labs(
    title = "Approximate Production-Specific Risk in Top-Risk Counties",
    x = "Production type",
    y = expression("Expected new infectious farms (" * Delta * I * ")"),
    fill = "Production"
  ) +
...
p_dom_species <- ggplot(counties_rich) +
  ...
  scale_fill_viridis_c(
    option = "C",
    name = expression("New infections (" * Delta * I * ")")
  ) +
  labs(
    title = "Predicted County Burden and Dominant Species",
    x = NULL,
    y = NULL
  ) +
  theme_pub() +
...
p_dom_prod <- ggplot(counties_rich) +
  ...
  labs(
    title = "Predicted County Burden and Dominant Production Type",
    x = NULL,
    y = NULL
  ) +
  theme_pub() +
